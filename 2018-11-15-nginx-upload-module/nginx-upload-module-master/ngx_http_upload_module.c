...
ngx_http_upload_handler(ngx_http_request_t *r)
{
    ...
    u = ngx_http_get_module_ctx(r, ngx_http_upload_module); /* 分配upload module上下文 */
    ...
    rc = upload_parse_request_headers(u, &r->headers_in); /* 解析出boundary出来 */
    ...
    if (ngx_http_upload_test_expect(r) != NGX_OK) { /* 客户端请求会包含一个Expect: 100的头部，这时服务器要返回一个100的http响应以表示服务器接受请求，这样客户端才会开始传输request body */
        upload_shutdown_ctx(u);
        return NGX_HTTP_INTERNAL_SERVER_ERROR;
    }

    if(upload_start(u, ulcf) != NGX_OK) /* 分配buffer (header accumulate buffer, output buffer) */
        return NGX_HTTP_INTERNAL_SERVER_ERROR;
    ...
    rc = ngx_http_read_upload_client_request_body(r); /* 读取request body */
...
static ngx_int_t ngx_http_upload_start_handler(ngx_http_upload_ctx_t *u) { /* {{{ */
    ...
    if(u->is_file) {
        ...
        u->cln = ngx_pool_cleanup_add(r->pool, sizeof(ngx_upload_cleanup_t)); /* 文件清理，注册以后，nginx会在请求处理结束阶段执行所注册的clean up handler，如删除文件 */
        ...
        if(u->session_id.len != 0) { /* 断点续传的逻辑，没仔细读，不讲解 */
            ...
            for(;;) {		/* 一个for循环尝试创建文件 (有可能会失败，所以要用一个循环) */
                ...
                file->fd = ngx_open_tempfile(file->name.data, 1, ulcf->store_access); /* 创建文件 */
        ...
        if(ulcf->field_templates) { /* 重新生成part的form field，比如文件路径、摘要 */
...
ngx_http_read_upload_client_request_body(ngx_http_request_t *r) {
    ...
    if (preread) {		/* 在还没进入到nginx upload module之前，nginx已经在读headers时顺便读入了一些request body */
        ...
        if (preread >= r->headers_in.content_length_n) { /* 读headers时正好把整个reqeust body都已经读完了 */
        ...
        } /* 否则，要额外开辟空间继续读取剩余的request body，这里不详细讲 */
    ...
    *next = cl;
    ...
    r->read_event_handler = ngx_http_read_upload_client_request_body_handler; /* 也许无法一次处理完请求，设置read handler以便之后 resume，其主要逻辑还是调用ngx_http_do_read_upload_client_request_body */

    return ngx_http_do_read_upload_client_request_body(r); /* 真正读取数据并且处理 */
} /* }}} */
...
ngx_http_do_read_upload_client_request_body(ngx_http_request_t *r)
{
    ...
    for ( ;; ) {				 /* 不断去处理buffer */
      for ( ;; ) {				 /* 不断去读数据 */
            if (rb->buf->last == rb->buf->end) { /* 读到一个完整的buffer */
            ...
            if (rb->rest == 0) { /* request body完全读取 */
                break;
            }

            if (rb->buf->last < rb->buf->end) { /* 所读数据达不到一个完整的buffer */
                break;
            }
    ...
    } /* 在完整读取到request body才会执行后面的逻辑，否则都在前面的return NGX_AGAIN结束处理，继续等待下次可读事件的发生 */

    if (c->read->timer_set) {
        ngx_del_timer(c->read);
    }

    rc = ngx_http_process_request_body(r, u->to_write); /* 对request body的最后一个buffer进行处理 */
...
static void upload_init_ctx(ngx_http_upload_ctx_t *upload_ctx) { /* {{{ */
     ...
     */
    upload_ctx->data_handler = upload_process_buf; /* 处理buffer */
} /* }}} */
...
static ngx_int_t upload_process_buf(ngx_http_upload_ctx_t *upload_ctx, u_char *start, u_char *end) { /* {{{ */
	...
	if(*p == *upload_ctx->boundary_pos) /* 逐个字符匹配boundary */
	  upload_ctx->boundary_pos++;
	else
	  upload_ctx->boundary_pos = upload_ctx->boundary_start; /* 不匹配，重启匹配过程 */

	if(upload_ctx->boundary_pos == upload_ctx->boundary.data + upload_ctx->boundary.len) { /* 匹配boundary，状态变更 */
	...
	switch(*p) {
	case '\n':		/* 出现换行，状态变更，准备解析part headers */
	  upload_ctx->state = upload_state_headers;
	  upload_ctx->header_accumulator_pos = upload_ctx->header_accumulator;
	case '\r':
	  break;
	case '-':
	  upload_ctx->state = upload_state_finish; /* 匹配boundary之后，出现了--，表示所有part都解析完 */
	  break;
	}
	break;
	/*
	...
	switch(*p) {
	case '\n':
	  if(upload_ctx->header_accumulator_pos == upload_ctx->header_accumulator) { /* 空header表示part headers解析完全 */
	    upload_ctx->is_file = (upload_ctx->file_name.data == 0) || (upload_ctx->file_name.len == 0) ? 0 : 1;

	    rc = upload_start_file(upload_ctx); /* 创建文件，准备response headers，后面会讲 */
                            
	    if(rc != NGX_OK) {
	      upload_ctx->state = upload_state_finish;
	      return rc; // User requested to cancel processing
	    } else {
	      upload_ctx->state = upload_state_data; /* 状态变更，准备处理data */
	      upload_ctx->output_buffer_pos = upload_ctx->output_buffer;	
	    }
	  } else {
	    *upload_ctx->header_accumulator_pos = '\0';
	    ...
	    rc = upload_parse_part_header(upload_ctx, (char*)upload_ctx->header_accumulator,
					  (char*)upload_ctx->header_accumulator_pos); /* 单个header读取完成，根据Content-Disposition判断是文件还是普通的part (key value) */

	    if(rc != NGX_OK) {
	      upload_ctx->state = upload_state_finish;
	      return rc; // Malformed header
	    } else
	      upload_ctx->header_accumulator_pos = upload_ctx->header_accumulator; /* 当前header处理完成，为读取下一个header做准备 */
	  }
	case '\r':
	  break;
	default:
	...
	if(*p == *upload_ctx->boundary_pos) /* 逐个字符匹配boundary */
	  ...
	  if(upload_ctx->boundary_pos == upload_ctx->boundary_start) {
	    ...
	    if(*p == '\n') {
	      ...
	      upload_putc(upload_ctx, *p); /* 把当前字符推到output_buffer中，upload_putc会在output_buffer满了的时候写出到文件 */
	  } else {	  /* 之前已经匹配了部分boundary，后来发现它其实是数据内容来的，要将这个部分boundary putc出去 */
	...
	if(upload_ctx->boundary_pos == upload_ctx->boundary.data + upload_ctx->boundary.len) { /* 匹配了完整的boundary */
	  upload_ctx->state = upload_state_after_boundary;
	  upload_ctx->boundary_pos = upload_ctx->boundary_start;

	  upload_flush_output_buffer(upload_ctx); /* 将之前缓存在output_buffer中还没写出的数据，写出到文件 */
	  if(!upload_ctx->discard_data)
	    upload_finish_file(upload_ctx); /* finish part，只是单个part完成解析，做一些收尾工作，比如计算md5摘要 */
	  else
	    upload_abort_file(upload_ctx);
	}
	break;
