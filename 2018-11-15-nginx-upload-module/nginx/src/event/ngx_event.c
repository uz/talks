...
ngx_process_events_and_timers(ngx_cycle_t *cycle)
{
    ...
    if (delta) {
        ngx_event_expire_timers(); /* 检查是否有定时器需要被触发 */
    }
...
ngx_event_process_init(ngx_cycle_t *cycle)
{
    ...
    for (m = 0; cycle->modules[m]; m++) {
        ...
        if (module->actions.init(cycle, ngx_timer_resolution) != NGX_OK) { /* 找到所使用的io复用机制，调用对应事件模块的初始化函数 (linux创建epoll instance) */
            /* fatal */
            exit(2);
        }
    ...
    ls = cycle->listening.elts;
    for (i = 0; i < cycle->listening.nelts; i++) { /* 每个监听的端口都给分配一个connection，connection描述了读写事件的处理方法，实际上在nginx中，每个socket都会分配一个connection结构体 */
        ...
        rev->handler = (c->type == SOCK_STREAM) ? ngx_event_accept /* 设置读事件handler */
                                                : ngx_event_recvmsg;
