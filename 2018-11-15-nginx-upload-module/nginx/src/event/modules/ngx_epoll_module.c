...
ngx_epoll_process_events(ngx_cycle_t *cycle, ngx_msec_t timer, ngx_uint_t flags)
{
    ...
    events = epoll_wait(ep, event_list, (int) nevents, timer); /* 阻塞等待网络事件 */
    ...
    for (i = 0; i < events; i++) { /* 遍历事件 */
        ...
        if ((revents & EPOLLIN) && rev->active) {
            ...
            if (flags & NGX_POST_EVENTS) { /* 加入到延后处理队列，因为accept是最优先的 */
