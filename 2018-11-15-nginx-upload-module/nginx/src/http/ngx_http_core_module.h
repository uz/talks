...
typedef enum {
    ...
    NGX_HTTP_SERVER_REWRITE_PHASE, /* server lookup (根据请求域名，ip和端口确定 server block) */

    NGX_HTTP_FIND_CONFIG_PHASE,	/* location lookup */
    ...
    NGX_HTTP_CONTENT_PHASE,	/* 第三方模块主要嵌入到这个阶段，因为到这个阶段时，nginx已经根据request line和request headers确定好了location */
