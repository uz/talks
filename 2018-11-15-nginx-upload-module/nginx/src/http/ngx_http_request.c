...
ngx_http_init_connection(ngx_connection_t *c)
{
    ...
    rev = c->read;
    rev->handler = ngx_http_wait_request_handler; /* 设置读事件handler */
    ...
    if (rev->ready) {		/* 读事件就绪，马上执行handler */
        /* the deferred accept(), iocp */

        if (ngx_use_accept_mutex) { /* 自身抢到了accept锁，则延迟执行handler，优先accept connection */
    ...
    if (ngx_handle_read_event(rev, 0) != NGX_OK) { /* 注册读事件的监听 */
        ngx_http_close_connection(c);
        return;
    }
}
